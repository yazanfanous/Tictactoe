...
public class Tictactoe {
            ...
            buttons[i].addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    ...
                    if (button.getText().isEmpty()) { // Check if the button is empty
                        if (xTurn) {    
                            button.setText("X"); //Places X if its their turn
                        } else {
                            button.setText("O"); //Places O if its their turn
                        }
                        button.setEnabled(false);
                        xTurn = !xTurn; //Disable the button after it is clicked so that only one thing can be placed on the square

                        if (isPlayerVsAI && !xTurn) {
                            makeAIMove(); //Make AI move when its their turn
                        }

                        checkForWinner(); //Check for winner after each loop
                    }
                }
            });
    ...
    private void makeAIMove() {
        ...
        // Method to make AI select a random button
        int index;
        do {
    ...
    private JPanel createExitPanel() {
        ...
        //Create a button that sends the user back to the intro screen
